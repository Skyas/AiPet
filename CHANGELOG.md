...
### Added
- **多厂商 AI 配置**：各厂商 API Key 和模型独立存储，切换厂商时自动恢复，互不覆盖
- **新增厂商预设**：豆包（火山引擎）、千问（阿里云 DashScope）
- **模型预设按钮**：每个厂商内置 2 个常用模型一键切换，支持「自定义」按钮进入自由输入模式，预设模型只读不可误改
- **服务商 Key 状态指示**：服务商按钮右下角绿点，已配置 Key 的厂商一目了然
- **主界面底部工具栏**：输入 / 截图 / 陪玩 / 语音（占位）/ QQ（占位）五个图标按钮
- **输入框按需展开**：默认收起，点击输入图标才滑入，再次点击收起
- **陪玩模式入口**：🎮 图标直接控制主动互动引擎开关，激活时青蓝发光，状态栏同步显示

### Changed
- 截图分析结果直接融入聊天流，不再跳转 VisionPanel
- 重载按钮旋转方向修正，新增 `blockSave` 防止 reload 期间响应式赋值误触发保存
- 退出设置页时仅在有实际改动（`isDirty`）时才落盘，不再无条件保存
- 切换厂商、选择预设模型改用静默保存，不弹 Toast
- Gemini 预设模型更新为 `gemini-2.5-flash` / `gemini-2.5-pro`
