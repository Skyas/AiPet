...
### Added
- **悬浮球模式**：点击「● 悬浮」按钮将主窗口缩小为桌面悬浮球，占用极小屏幕空间
- **漫画气泡通知**：AI 主动消息 / 截图分析结果以漫画对话框形式弹出，显示后自动消失，完全穿透鼠标点击不影响游戏操作
- **动态显示时长**：气泡停留时间根据文字长度动态计算（基础 3s + 每 8 字 1s，上限 20s），并附进度条
- **气泡智能定位**：默认显示在悬浮球上方，空间不足时自动切换到下方，尾巴位置跟随球体实时计算
- **悬浮球拖动**：可拖动到屏幕任意位置，位置持久化，下次启动自动恢复
- **悬浮球 DPI 适配**：按系统缩放比例（scaleFactor）自动调整球体尺寸（52 / 56 / 62px）
- **悬停工具栏**：鼠标悬停在悬浮球上方时弹出工具栏，提供展开 / 截图 / 陪玩 / 语音占位 / QQ 占位五个操作
- **双击展开**：双击悬浮球还原主窗口，防止游戏中误触

### Changed
- 主窗口标题栏按钮统一为带文字标签的胶囊样式（清空 / 陪玩 / 设置 / 悬浮），移除纯图标按钮
- 去掉「收起」按钮，改为在主窗口任意位置右键弹出系统菜单（收起窗口 / 清空对话）
- 悬浮球拖动改为主进程 `getCursorScreenPoint()` 轮询方案，彻底解决快速移动时脱钩问题
- `router.js` 新增 `/pet-ball` 和 `/bubble` 两条路由

### Fixed
- 拖动悬浮窗时鼠标移动过快导致脱钩（根本原因：渲染层 mousemove 在鼠标离开窗口后停止触发）
